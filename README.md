...
A local-first dashboard for teachers: track students, classes, lesson plans, grades, attendance, and reports across K-12, university, and club sections — from one desktop app that runs entirely offline, with no account, no server, and no internet required.
...
| Gradebook | Attendance | Class report |
| --- | --- | --- |
| ![Gradebook](docs/screenshots/gradebook.png) | ![Attendance](docs/screenshots/attendance.png) | ![Class report](docs/screenshots/class-report.png) |

## Download

Grab the latest build for your platform from the [Releases page](https://github.com/purysho/EduBoard/releases):

- **Windows** — `EduBoard-<version>-portable.exe`. No install, no admin rights: copy it to a USB stick and double-click it anywhere. EduBoard creates an `EduBoard-data` folder next to the exe and keeps your database there, so the whole thing travels together. (An installer, `EduBoard-<version>-setup.exe`, is also published if you'd rather install it normally — in that case your data lives in your Windows user profile instead.)
- **macOS** — `eduboard-<version>.dmg` (or the `.zip`). Universal build, works on Intel and Apple Silicon.
- **Linux** — `eduboard-<version>.AppImage`.

> Builds aren't code-signed (that costs money neither of us needs to spend for a personal tool). Windows SmartScreen and macOS Gatekeeper will warn you the first time you open it — on Windows choose **More info → Run anyway**; on macOS right-click the app → **Open** the first time.
...
3. On a class page: **Roster** to enroll students, **Gradebook** to add assessments and enter scores, **Attendance** to mark the day, **Lesson plans** to jot down what you're teaching, **Report** for the class-wide picture and printable report cards.
4. **Settings → Backups** → back up before anything risky (a big import, a term rollover) and occasionally copy the backup file somewhere off the laptop.
