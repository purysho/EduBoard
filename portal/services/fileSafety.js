// Plain-JS copy of src/shared/fileSafety.ts (the Portal has no build step): an upload
// must have an allowed extension, contents matching that type, no executable signature,
// and no macro project. Both copies run against src/shared/__tests__/
// fileSafetyCases.json; change them together.
...
function startsWith(b, bytes, offset = 0) {
...
const utf16le = (s) => Array.from(s).flatMap((c) => [c.charCodeAt(0), 0])

function executableSignature(b) {
...
function checkUpload(fileName, bytes) {
  const ext = (/\.([a-z0-9]+)$/i.exec(fileName)?.[1] ?? '').toLowerCase()
  if (!SUBMISSION_EXTENSIONS.includes(ext)) {
  ...
  const mismatch = { ok: false, reason: `the file's contents don't match ".${ext}"` }
...
module.exports = { checkUpload, SUBMISSION_EXTENSIONS }
